...
def is_server_side_tool_error_part(part: dict[str, object]) -> bool:
    """True when a part is the web backend's own tool-execution failure (model 'all-tools').

    When the model calls a tool that the client should execute (bash, write,
    todowrite, ...), the web backend still tries to execute it server-side and
    reports the failure as a role='tool' part with status='error'. For our
    purposes that error is expected: the tool call itself (in the preceding
    assistant part) is relayed to the client, which executes it. It must not
    abort the stream.
    """
    if str(part.get("role", "")).strip().lower() != "tool":
        return False
    model = str(part.get("model", "")).strip()
    meta = part.get("meta_data")
    show_type = str(meta.get("show_type", "")).strip() if isinstance(meta, dict) else ""
    return model == "all-tools" or show_type.startswith("mc_tool_result")
...
class GLMWebClient:
    ...
    @staticmethod
    def _tool_schemas_from(filtered_tools: list[dict[str, object]] | None) -> dict[str, dict[str, object]] | None:
    ...
    def _raise_for_event_error(self, event: dict[str, object], stream: bool) -> None:
        ...
        if not error_payload and status == "error" and self._only_server_side_tool_errors(event):
            self.logger.warning("忽略 all-tools 服务端工具执行失败（工具将由客户端执行） event_status=error")
            return
    ...
    def _only_server_side_tool_errors(self, event: dict[str, object]) -> bool:
        ...
        for part in parts:
            if not isinstance(part, dict):
                continue
            if is_server_side_tool_error_part(part):
                continue
            error = part.get("error")
            part_status = str(part.get("status", "")).strip().lower()
            if (isinstance(error, dict) and error) or part_status == "error":
                return False
        return True
