...
from ..utils.tool_parser import StreamingToolParser, coerce_tool_arguments, parse_tool_calls_from_text
...
class GLMEventAccumulator:
    ...
    def finalize(self, status: str | None, last_error: dict[str, object] | None = None) -> list[str]:
        ...
        all_tool_calls = self._coerce_tool_call_arguments(all_tool_calls)
    ...
    def _coerce_tool_call_arguments(self, tool_calls: list[dict[str, object]]) -> list[dict[str, object]]:
        """Best-effort decode stringified array/object arguments per the tool schemas.

        The upstream model (and the web backend's own DSML parser) frequently
        returns parameters that the schema declares as array/object as
        JSON-encoded strings. Strict clients reject those with SchemaError, so
        we decode them back before relaying the tool call downstream.
        """
        ...
        for tool_call in tool_calls:
            ...
            if schema is None:
                continue
            fn["arguments"] = coerce_tool_arguments(name, fn.get("arguments"), schema)
        return tool_calls
