...
_XML_TAG_PATTERN = re.compile(
    r'<(/?)([A-Za-z_][A-Za-z0-9_.:-]*)((?:\s+[\w.:-]+(?:\s*=\s*"[^"]*")?)*)\s*(/?)>',
    re.IGNORECASE,
)


def _drop_unmatched_close_tags(xml: str) -> str:
    """Remove closing tags that have no matching open tag.

    Models occasionally hallucinate stray closers such as ``</arg_value>``
    inside a parameter, which makes ElementTree reject the whole block.
    Only invoked on the repair path (initial parse already failed).
    """
    stack: list[str] = []
    out: list[str] = []
    last = 0
    for match in _XML_TAG_PATTERN.finditer(xml):
...
ITEM_ENTRY_PATTERN = re.compile(r"<item\b[^>]*>(.*?)</item\s*>", re.IGNORECASE | re.DOTALL)


def _parse_item_list_text(text: str) -> list[object] | None:
    """Recover arrays the model wrapped as CDATA text: '<item>a</item>,<item>b</item>'."""
    matches = ITEM_ENTRY_PATTERN.findall(text)
    if not matches:
        return None
    skeleton = ITEM_ENTRY_PATTERN.sub("", text).strip()
    if re.fullmatch(r"[\s,;]*", skeleton) is None:
        return None
    return [_coerce_leaf_value(raw) for raw in matches]
...
def _coerce_leaf_value(text: str) -> object:
    ...
    if re.fullmatch(r"-?\d+\.\d+", stripped):
        try:
            ...
            return stripped
    # Some models double-encode JSON values as a quoted string, e.g. '"[{\"a\":1}]"'.
    if len(stripped) >= 2 and stripped.startswith('"') and stripped.endswith('"'):
        try:
            inner = json.loads(stripped)
        except json.JSONDecodeError:
            inner = None
        if isinstance(inner, str) and inner != stripped:
            coerced = _coerce_leaf_value(inner)
            if coerced != inner:
                return coerced
    # Some models wrap array parameters in CDATA as text instead of real <item> elements.
    if "<item" in stripped.lower():
        items = _parse_item_list_text(stripped)
...
def _parse_xml_block(
    ...
    try:
        ...
        try:
            root = ET.fromstring(_drop_unmatched_close_tags(normalized))
        except ET.ParseError:
            return [], None
...
def _coerce_to_schema_type(value: object, key_schema: dict[str, object]) -> object:
    """Coerce a single argument value toward the type declared by the tool schema.

    Models (and the web backend's DSML parser) regularly return values that the
    schema declares as array/object as JSON-encoded strings, e.g. '"todos": "[{...}]"'.
    Strict clients (OpenCode's zod, Cherry Studio, ...) reject those with
    SchemaError, so we decode them back into real structures when possible.
    """
    if not isinstance(value, str):
        return value
    expected = key_schema.get("type")
    if isinstance(expected, list):
        expected = next((t for t in expected if t in {"array", "object", "integer", "number", "boolean"}), None)
    expected = str(expected).strip().lower() if expected is not None else ""
...
def coerce_tool_arguments(name: str, arguments: object, schema: dict[str, object] | None) -> str:
    """Return tool-call arguments as a JSON string coerced toward the tool schema.

    Accepts a JSON string or a dict (both appear in the wild); returns the
    canonical compact JSON string form used for OpenAI tool_calls.
    """
        ...
        if isinstance(props, dict):
            for key, value in list(args.items()):
                ...
                if isinstance(key_schema, dict):
                    args[key] = _coerce_to_schema_type(value, key_schema)
