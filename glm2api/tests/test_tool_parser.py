...
def test_coerce_tool_arguments_decodes_stringified_structures():
    from glm2api.utils.tool_parser import coerce_tool_arguments

    # stringified array (todowrite incident)
    out = coerce_tool_arguments("todowrite", '{"todos": "[{\\"a\\":1}]"}', {"type": "object", "properties": {"todos": {"type": "array"}}})
    assert json.loads(out) == {"todos": [{"a": 1}]}

    # <item> list wrapped as one string (question incident)
    out = coerce_tool_arguments("question", '{"questions": "<item>{\\"q\\":1}</item>,<item>{\\"q\\":2}</item>"}', {"type": "object", "properties": {"questions": {"type": "array"}}})
    assert json.loads(out) == {"questions": [{"q": 1}, {"q": 2}]}

    # stringified object
    out = coerce_tool_arguments("y", '{"meta": "{\\"k\\": 1}"}', {"type": "object", "properties": {"meta": {"type": "object"}}})
    assert json.loads(out) == {"meta": {"k": 1}}

    # scalars coerced per schema type
    out = coerce_tool_arguments("z", '{"multiple": "true", "n": "3"}', {"type": "object", "properties": {"multiple": {"type": "boolean"}, "n": {"type": "integer"}}})
    assert json.loads(out) == {"multiple": True, "n": 3}

    # string-typed params stay strings
    out = coerce_tool_arguments("s", '{"id": "123"}', {"type": "object", "properties": {"id": {"type": "string"}}})
    assert json.loads(out) == {"id": "123"}

    # no schema -> semantically unchanged
    out = coerce_tool_arguments("x", '{"todos": "[1]"}', None)
    assert json.loads(out) == {"todos": "[1]"}

    # dict input is accepted too
    out = coerce_tool_arguments("d", {"todos": '[{"a":1}]'}, {"type": "object", "properties": {"todos": {"type": "array"}}})
    assert json.loads(out) == {"todos": [{"a": 1}]}
